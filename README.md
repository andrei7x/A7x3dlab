...
ADMIN_SETUP_TOKEN=troque-este-token-longo-de-setup
...
Crie ou atualize o primeiro admin chamando a rota protegida de setup depois de configurar o Supabase e as variaveis de ambiente:
...
curl -X POST http://localhost:3000/api/admin/setup \
  -H "Authorization: Bearer seu-admin-setup-token"
```

Exemplo em producao:

```bash
curl -X POST https://seu-site.netlify.app/api/admin/setup \
  -H "Authorization: Bearer seu-admin-setup-token"
```

Se o admin ja existir, a rota atualiza `password_hash`, incrementa `password_version` e `session_version`, define `password_changed_at` e invalida sessoes antigas. A senha sempre e salva como hash bcrypt, nunca em texto puro.
